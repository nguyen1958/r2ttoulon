...
public class GestionRapportComptableAnnuel extends fr.analogon.r2t.main.RacineServlet 
{
	...
	{		
		debug.logger.debug("Batch raport comptable annuel............. ");
		...
		if(choix.equalsIgnoreCase("lancer"))
		{
			...
			debug.logger.debug("lancement raport comptable annuel pour l'annee ="+annee);
			...
			response.sendRedirect("./entree?action=gestionBatchRapportsComptableAnnuel.jsp&numeroBatch="+numeroBatch);
			
		}
		//Consulttion
		else
		{
			debug.logger.debug("recuperation raport comptable annuel");
			response.sendRedirect("./entree?action=gestionBatchRapportsComptableAnnuel.jsp&numBatchRapport=" );
		}
	}
